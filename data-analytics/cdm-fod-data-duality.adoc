...
Necesita NetApp BlueXP, NetApp Cloud Volumes ONTAP y AWS SageMaker Notebooks para el caso de uso de dualidad de datos.



=== Requisitos de software

La siguiente tabla enumera los componentes de software necesarios para implementar el caso de uso.
...
| Volúmenes en la nube de NetApp Cloud Volumes ONTAP | 1 


| Cuaderno de AWS SageMaker | 1 
|===


== Procedimientos de implementación

La implementación de la solución de dualidad de datos implica las siguientes tareas:

* Conector BlueXP
* Volúmenes en la nube de NetApp Cloud Volumes ONTAP
* Datos para el aprendizaje automático
* AWS SageMaker
* Aprendizaje automático validado desde Jupyter Notebooks
...
. Utilizamos las credenciales basadas en mcarl-marketplace-subscription en BlueXP.
. Elija la región adecuada para su entorno (por ejemplo, us-east-1 [N. Virginia]) y seleccione el método de autenticación (por ejemplo, Asumir rol o claves de AWS).  En esta validación, utilizamos claves de AWS.
. Proporcione el nombre del conector y cree un rol.
. Proporcione los detalles de la red, como la VPC, la subred o el par de claves, dependiendo de si necesita una IP pública o no.
. Proporcione los detalles del grupo de seguridad, como acceso HTTP, HTTPS o SSH desde el tipo de origen, como cualquier lugar e información de rango de IP.
. Revise y cree el conector BlueXP .
. Verifique que el estado de la instancia EC2 de BlueXP se esté ejecutando en la consola de AWS y verifique la dirección IP en la pestaña *Redes*.
. Inicie sesión en la interfaz de usuario del conector desde el portal BlueXP o puede utilizar la dirección IP para acceder desde el navegador.
...
Para crear una instancia de Cloud Volumes ONTAP en BlueXP, complete los siguientes pasos:

. Cree un nuevo entorno de trabajo, seleccione el proveedor de nube y seleccione el tipo de instancia de Cloud Volumes ONTAP (como CVO único, HA o Amazon FSx ONTAP para ONTAP).
. Proporcione detalles como el nombre y las credenciales del clúster de Cloud Volumes ONTAP .  En esta validación, creamos una instancia de Cloud Volumes ONTAP llamada `svm_sagemaker_cvo_sn1` .
...
. En la sección *Ubicación y conectividad*, seleccione la región de AWS, la VPC, la subred, el grupo de seguridad, el método de autenticación SSH y una contraseña o un par de claves.
. Elija el método de carga.  Utilizamos *Profesional* para esta validación.
. Puede elegir un paquete preconfigurado, como *POC y cargas de trabajo pequeñas*, *Cargas de trabajo de producción de datos de aplicaciones y bases de datos*, *DR rentable* o *Cargas de trabajo de producción de mayor rendimiento*.  En esta validación elegimos *Poc y cargas de trabajo pequeñas*.
. Cree un volumen con un tamaño específico, protocolos permitidos y opciones de exportación.  En esta validación, creamos un volumen llamado `vol1` .
. Elija un tipo de disco de perfil y una política de niveles.  En esta validación, deshabilitamos *Eficiencia de almacenamiento* y *SSD de propósito general – Rendimiento dinámico*.
. Por último, revise y cree la instancia de Cloud Volumes ONTAP .  Luego espere entre 15 y 20 minutos para que BlueXP cree el entorno de trabajo de Cloud Volumes ONTAP .
. Configure los siguientes parámetros para habilitar el protocolo Duality.  El protocolo Duality (NFS/S3) es compatible con ONTAP 9.  12.1 y posteriores.
+
.. En esta validación, creamos un SVM llamado `svm_sagemaker_cvo_sn1` y volumen `vol1` .
.. Verifique que la SVM tenga soporte de protocolo para NFS y S3.  En caso contrario, modifique el SVM para admitirlos.
+
....
...
. Cree e instale un certificado CA si es necesario.
...
sagemaker_cvo_sn1::*>
....
. Verifique los detalles agregados.
+
....
...
sagemaker_cvo_sn1::*>
....
. Cree un depósito en el volumen NFS.
+
....
...
Para crear un cuaderno de AWS desde AWS SageMaker, complete los siguientes pasos:

. Asegúrese de que el usuario que está creando la instancia de Notebook tenga una política de IAM de AmazonSageMakerFullAccess o sea parte de un grupo existente que tenga derechos de AmazonSageMakerFullAccess.  En esta validación, el usuario es parte de un grupo existente.
. Proporcione la siguiente información:
+
** Nombre de la instancia del cuaderno.
** Tipo de instancia.
** Identificador de plataforma.
** Seleccione la función de IAM que tenga derechos AmazonSageMakerFullAccess.
** Acceso root – habilitar.
** Clave de cifrado: no seleccione ningún cifrado personalizado.
...
+
image:cdm-fod-002.png["Captura de pantalla que representa el paso."]

+
image:cdm-fod-003.png["Captura de pantalla que representa el paso."]

. Inicie el cuaderno de AWS.
+
image:cdm-fod-004.png["Captura de pantalla que representa el paso."]

. Abra el laboratorio de Jupyter.
+
image:cdm-fod-005.png["Captura de pantalla que representa el paso."]

. Inicie sesión en la terminal y monte el volumen Cloud Volumes ONTAP .
+
....
...
sh-4.2$
....
. Verifique el depósito creado en el volumen Cloud Volumes ONTAP mediante los comandos de AWS CLI.
+
....
...
En esta validación, utilizamos un conjunto de datos de DBpedia, un esfuerzo comunitario de colaboración colectiva, para extraer contenido estructurado de la información creada en varios proyectos de Wikimedia.

. Descargue los datos de la ubicación de GitHub de DBpedia y extráigalos.  Utilice la misma terminal utilizada en la sección anterior.
+
....
sh-4.2$ wget
--2023-02-14 23:12:11--
...
sh-4.2$
....
. Copie los datos en la ubicación de Cloud Volumes ONTAP y verifíquelos desde el depósito S3 mediante la AWS CLI.
+
....
sh-4.2$ df -h
Filesystem          Size  Used Avail Use% Mounted on
...
sh-4.2$
....
. Realice una validación básica para asegurarse de que la funcionalidad de lectura y escritura funcione en el bucket S3.
+
....
...
== Validar el aprendizaje automático desde Jupyter Notebooks

La siguiente validación permite construir, entrenar e implementar modelos de aprendizaje automático a través de la clasificación de texto utilizando el siguiente ejemplo de SageMaker BlazingText:
...
....
. En el siguiente paso, los datos(`dbpedia_csv` ) se descarga del depósito s3 `ontapbucket1` a una instancia de Jupyter Notebook utilizada en aprendizaje automático.
+
....
...
....
+
La salida enumera los archivos y carpetas en el `ontapbucket1` depósitos que se utilizan como datos para la validación de aprendizaje automático de AWS SageMaker.
...
....
. Cargue el conjunto de datos formateados y de entrenamiento en S3 para que SageMaker pueda usarlo para ejecutar trabajos de entrenamiento.  Luego, cargue dos archivos en el depósito y prefije la ubicación usando el SDK de Python.
+
....
ln [8]: %%time
...
Wall time: 1.32 s
....
. Configure una ubicación de salida en S3 donde se carga el artefacto del modelo para que los artefactos puedan ser la salida del trabajo de entrenamiento del algoritmo.  Crear una `sageMaker.estimator.Estimator` objeto para lanzar el trabajo de entrenamiento.
+
....
...
....
. Definir SageMaker `Estrimator` con configuraciones de recursos e hiperparámetros para entrenar la clasificación de texto en el conjunto de datos DBPedia utilizando el modo supervisado en una instancia c4.4xlarge.
+
....
...
....
. Preparar un protocolo de enlace entre los canales de datos y el algoritmo.  Para ello, cree el `sagemaker.session.s3_input` objetos de los canales de datos y los guarda en un diccionario para que el algoritmo los consuma.
+
....
...
....
. Una vez finalizado el trabajo, aparece un mensaje de Trabajo completado.  El modelo entrenado se puede encontrar en el bucket S3 que se configuró como `output_path` en el estimador.
+
....
...
Billable seconds: 68
....
. Una vez finalizado el entrenamiento, implemente el modelo entrenado como un punto final alojado en tiempo real de Amazon SageMaker para realizar predicciones.
+
....
...
....
. De forma predeterminada, el modelo devuelve una predicción con la mayor probabilidad.  Para recuperar la parte superior `k` predicciones, establecer `k` en el archivo de configuración.
+
....
...
....
. Elimine el punto final antes de cerrar el cuaderno.
+
....
